...
For HomeKit's linked climate fan, the integration exposes the [four standard fixed names that Home Assistant recognizes](https://github.com/home-assistant/core/blob/7c5eb60b7a98a3900cece9d8719f7a5620f459e7/homeassistant/components/homekit/climate_util.py#L27-L55). The Home app therefore renders `0/25/50/75/100%` as automatic and fixed levels `1/2/3/5`. HomeKit still labels 0% as off, but the integration deliberately translates that [linked-fan active event](https://github.com/home-assistant/core/blob/7c5eb60b7a98a3900cece9d8719f7a5620f459e7/homeassistant/components/homekit/climate_base.py#L298-L314) to automatic (`windSpeed=1`); the main climate power switch remains the only way to turn the room off. If the room is already off, selecting 0% only saves automatic as the next target and never turns it on. Fixed-level display comes from the latest server report rather than the requested target, so a rejected or unapplied command rolls the slider back after readback. Physical level 4 remains visible in `reported_fan_level` but is rendered at the nearer 75% HomeKit step.
...
- Because the cloud reports execution level but not target type, a wired-controller change from a fixed target to automatic cannot be distinguished from a change to the currently executing fixed level. Select the Home Assistant/HomeKit 0% step (automatic) once after making that particular external change.
